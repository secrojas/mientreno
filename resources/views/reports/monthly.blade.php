@php
    ...
    // Week-by-week breakdown dentro del mes
    $weeklyBreakdown = collect();
    $weekPtr = $period['start_date']->copy()->startOfWeek();
    while ($weekPtr->lte($period['end_date'])) {
        $weekEnd     = $weekPtr->copy()->endOfWeek();
        $rangeStart  = $weekPtr->lt($period['start_date']) ? $period['start_date']->copy() : $weekPtr->copy();
        $rangeEnd    = $weekEnd->gt($period['end_date']) ? $period['end_date']->copy() : $weekEnd->copy();
        $weekWorkouts = $workouts->filter(fn ($w) => $w->date->between($rangeStart, $rangeEnd));
        $weeklyBreakdown->push([
            'label'    => 'Sem ' . $weekPtr->isoWeek(),
            'start'    => $rangeStart,
            'end'      => $rangeEnd,
            'total_km' => (float) $weekWorkouts->sum('distance'),
            'sessions' => $weekWorkouts->count(),
        ]);
        $weekPtr->addWeek();
    }
    $maxWeekKm = max($weeklyBreakdown->max('total_km'), 0.01);
...
<x-app-layout title="Reporte Mensual">
    ...
    <div class="relative bg-bg-card border border-border-subtle rounded-card p-6 sm:p-8 mb-4 overflow-hidden">
        ...
        <div class="relative flex flex-col lg:flex-row items-start lg:items-center gap-6 lg:gap-10">
            ...
            <div class="flex-shrink-0">
                ...
                @if($hasTrend)
                    ...
                    <div class="mt-3 inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold"
                        ...
                        @else → @endif
                        {{ abs($distTrend['percentage']) }}% vs mes anterior
                    </div>
                @endif
            </div>
            ...
            {{-- Supporting stats --}}
            <div class="flex-1 grid grid-cols-2 sm:grid-cols-3 gap-4 sm:gap-6">
                <div>
                ...
                @if($summary['avg_heart_rate'])
                    <div class="border-l border-border-subtle pl-4 sm:pl-6 sm:col-start-2 lg:col-start-auto">
                        <div class="text-2xl sm:text-3xl font-display font-semibold" style="color: #EF4444">
                            {{ round($summary['avg_heart_rate']) }}
                        </div>
                        <div class="text-xs text-text-muted mt-1 uppercase tracking-wide">FC prom. bpm</div>
                    </div>
                @endif
                @if($summary['elevation_gain'])
                    <div class="border-l border-border-subtle pl-4 sm:pl-6">
                        <div class="text-2xl sm:text-3xl font-display font-semibold" style="color: #8B5CF6">
                            {{ number_format($summary['elevation_gain']) }}
                        </div>
                        <div class="text-xs text-text-muted mt-1 uppercase tracking-wide">Desnivel m+</div>
                    </div>
                @endif
            </div>
        </div>
    ...
    <div class="bg-bg-card border border-border-subtle rounded-card p-5 sm:p-6 mb-4">
        ...
        <div class="flex gap-3 sm:gap-5 items-end" style="height: 120px">
            @foreach($weeklyBreakdown as $week)
                @php
                    $barHeightPct = $week['total_km'] > 0
                        ? max(($week['total_km'] / $maxWeekKm) * 75, 5)
                        : 0;
                @endphp
                <div class="flex-1 flex flex-col items-center justify-end gap-0" style="height: 100%">
                    {{-- km label --}}
                    <div class="text-xs font-medium mb-1 {{ $week['total_km'] > 0 ? 'text-text-main' : 'text-transparent' }}"
                         style="font-feature-settings: 'tnum'">
                        {{ $week['total_km'] > 0 ? number_format($week['total_km'], 1) : '·' }}
                    </div>
                        ...
                        @if($week['total_km'] > 0)
                            <div class="w-full rounded-t-lg transition-all duration-300"
                                 style="height: {{ $barHeightPct }}%;
                                        background: linear-gradient(to top, #2DE38E, #60A5FA);
                                        box-shadow: 0 -4px 12px rgba(45,227,142,0.25);">
                            </div>
                        @else
                            <div class="w-full rounded-full" style="height: 2px; background-color: #111827;"></div>
                        @endif
                    </div>

                    {{-- Week label --}}
                    <div class="text-xs mt-1.5 uppercase font-medium text-text-muted">
                        {{ $week['label'] }}
                    </div>
                    @if($week['sessions'] > 0)
                        <div class="text-xs text-text-muted" style="font-size: 10px">
                            {{ $week['sessions'] }} ses.
                        </div>
                    @else
    ...
    {{-- ═══════════════════════════════ --}}
    @if($hasTrend)
        <x-card title="Comparativa con mes anterior" class="mb-4">
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <div class="flex items-center gap-3">
                                    ...
                                    <span class="text-xs text-text-muted flex-shrink-0">
                                        {{ $data['count'] }} ses. · {{ number_format($data['distance'], 1) }} km
                                    </span>
                                </div>
                            </div>
                        </div>
